...
interface AddToCartActionsProps {
  product: Product;
  userRole: string; // Ej. 'Cliente', 'Auditor'
}
...
export const AddToCartActions = ({ product, userRole }: AddToCartActionsProps) => {
  ...
  if (userRole === 'Auditor') {
    return null; // Oculta completamente los controles de compra
  }

  const handleAdd = async () => {
    if (quantity > 0) {
      const result = await addProductToCart(product, quantity);
      // Escenario 1: Muestra un mensaje confirmando que el producto fue añadido
